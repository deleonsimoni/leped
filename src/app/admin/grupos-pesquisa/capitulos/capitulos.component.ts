...
export class CapitulosComponent implements OnInit {
  ...
  private listAll(): Observable<any> {
    return this.grupoPesquisaService.listCapitulo()
      .pipe(
    ...
    this.openDialog()
      .pipe(
        switchMap(({ save, form, file }: any) =>
          iif(() => save,
            this.grupoPesquisaService.cadastrarCapitulo(form)
    ...
    this.openDialog(data)
      .pipe(
        switchMap(({ save, form }: any) =>
          iif(() => save,
            this.grupoPesquisaService.atualizarCapitulo({ ...form, _id: data._id })
              .pipe(switchMap(_ => this.listAll())),
            of(null)
          )
        )
  ...
  public delete(item: any): void {
    this.grupoPesquisaService.deletarCapitulo(item._id)
      .pipe(
        take(1),
